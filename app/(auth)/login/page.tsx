  ...
  async function handleLogin(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await signIn("credentials", {
        email: email,
        password: password,
        redirect: false, // será feito mais a baixo, pois signIn não tem throw para erros 
      })

      if (result?.error) {
        let errorMessage = "Erro ao fazer login."
        // "CredentialsSignin" é o código padrão para erro de senha/email
        ...
        <CardContent className="space-y-4">
          ...
          >
            ...
            >
              {isLoading ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Efetuado login...
                </>
              ) : (
                "Entrar"
              )}
